...
than a sequence of characters, the tool does not try to analyze
...
in Arabic using two separate tokens, ʿAbd and Allāh, and both 
...
This means that when you're interested in how many times an
...
in exactly the same way in unvowelled Arabic (كتب), and
...
could give us an idea of what tokens in our texts are probably
errors. We are currently experimenting with the use of such frequency 
lists of single tokens and token bigrams (combinations of two adjacent 
tokens) extracted from the entire OpenITI corpus for identifying 
possible OCR mistakes in texts produced by the OCR pipeline 
we are setting up. More about this in a next blog!
